...
# ================================
# Heat transfer core equations
# ================================
def heat_from_mdot(mdot, cp, dt):
    return mdot * cp * dt

def mdot_from_heat(Q, cp, dt):
    if cp <= 0 or dt == 0:
        raise ValueError("Cp must be >0 and ΔT cannot be zero")
    return Q / (cp * dt)

def vdot_from_heat(Q, cp, dt, rho):
    mdot = mdot_from_heat(Q, cp, dt)
    if rho <= 0:
